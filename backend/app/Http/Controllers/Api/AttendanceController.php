...
class AttendanceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // ✅ NOTIFIKASI: Kirim ke atasan jika terlambat
        $checkInTime = Carbon::parse($attendance->check_in);
        $scheduledTime = Carbon::parse($today->format('Y-m-d') . ' ' . $attendance->jadwal_checkin);
        
        if ($checkInTime->gt($scheduledTime)) {
            $lateMinutes = $checkInTime->diffInMinutes($scheduledTime);
            
            // Kirim notifikasi ke atasan jika user punya atasan
            if ($request->user()->atasan_id) {
                NotificationHelper::create(
                    $request->user()->atasan_id,
                    'attendance_late',
                    'Karyawan Terlambat',
                    "{$request->user()->name} check-in terlambat {$lateMinutes} menit pada {$today->format('d M Y')}.",
                    [
        ...
        // ✅ NOTIFIKASI: Kirim ke HR/Admin jika out of office
        if ($request->out_of_office) {
            $hrAdmins = User::role(['Admin', 'HR'])->pluck('id')->toArray();
            NotificationHelper::createForMultiple(
                $hrAdmins,
                'attendance_out_of_office',
                'Check-in Di Luar Kantor',
                "{$request->user()->name} melakukan check-in di luar kantor. Alasan: {$request->out_of_office_reason}",
                [
                    'attendance_id' => $attendance->id,
                    'employee_name' => $request->user()->name,
                    'reason' => $request->out_of_office_reason
                ]
            );
        }
    ...
    public function checkOut(Request $request, $id)
    {
        ...
        // ✅ NOTIFIKASI: Hitung jam kerja dan kirim notifikasi jika lembur
        $checkInTime = Carbon::parse($attendance->check_in);
        $checkOutTime = Carbon::parse($attendance->check_out);
        $workHours = $checkOutTime->diffInHours($checkInTime);
        
        // Jika kerja lebih dari 9 jam (asumsi jam kerja normal 8 jam + 1 jam istirahat)
        if ($workHours > 9) {
            ...
            if ($request->user()->atasan_id) {
                NotificationHelper::create(
                    $request->user()->atasan_id,
                    'attendance_overtime_detected',
                    'Potensi Lembur Terdeteksi',
                    "{$request->user()->name} bekerja selama {$workHours} jam (potensi lembur {$overtimeHours} jam) pada {$attendance->tanggal->format('d M Y')}.",
                    [
                        'attendance_id' => $attendance->id,
                        'employee_name' => $request->user()->name,
        ...
        // ✅ NOTIFIKASI: Check-out terlalu cepat (kurang dari 8 jam)
        if ($workHours < 8) {
            // Kirim notifikasi ke atasan
            if ($request->user()->atasan_id) {
                NotificationHelper::create(
                    ...
                    "{$request->user()->name} check-out setelah bekerja hanya {$workHours} jam pada {$attendance->tanggal->format('d M Y')}.",
                    [
                        'attendance_id' => $attendance->id,
                        'employee_name' => $request->user()->name,
