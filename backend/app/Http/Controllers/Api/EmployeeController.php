...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Add additional data - gunakan accessor picture_url dari model
        $employees->getCollection()->transform(function($employee) {
            $employee->masa_kerja = $employee->getMasaKerja();
            // picture_url sudah otomatis dari accessor
            return $employee;
        });
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // ✅ NOTIFIKASI: Kirim ke karyawan baru
            NotificationHelper::create(
                $user->id,
                'employee_created',
                'Selamat Datang!',
                "Akun Anda telah dibuat. Selamat bergabung di {$user->company} sebagai {$user->posisi}.",
                ['employee_id' => $user->id, 'email' => $user->email]
            );

            // ✅ NOTIFIKASI: Kirim ke atasan jika ada
            if ($user->atasan_id) {
                NotificationHelper::create(
                    $user->atasan_id,
                    'new_bawahan',
                    'Bawahan Baru',
                    "{$user->name} telah ditambahkan sebagai bawahan Anda.",
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $oldAtasanId = $user->atasan_id;
            $oldPosisi = $user->posisi;
            ...
            // ✅ NOTIFIKASI: Kirim ke karyawan jika ada perubahan penting
            $changes = [];
            if ($oldPosisi !== $validated['posisi']) {
                $changes[] = "posisi dari {$oldPosisi} ke {$validated['posisi']}";
            }
            if ($oldRole !== $validated['role']) {
                $changes[] = "role dari {$oldRole} ke {$validated['role']}";
            }
            if ($oldCompany !== $validated['company']) {
                $changes[] = "perusahaan dari {$oldCompany} ke {$validated['company']}";
            }
            if ($oldAtasanId !== ($validated['atasan_id'] ?? null)) {
                $changes[] = "atasan";
            }
            ...
            if (!empty($changes)) {
                NotificationHelper::create(
                    $user->id,
                    'employee_updated',
                    'Data Anda Diperbarui',
                    "Data Anda telah diperbarui: " . implode(', ', $changes) . ".",
                    ['employee_id' => $user->id, 'changes' => $changes]
                );
            }

            // ✅ NOTIFIKASI: Kirim ke atasan baru jika berubah
            if ($oldAtasanId !== ($validated['atasan_id'] ?? null) && $validated['atasan_id']) {
                NotificationHelper::create(
                    $validated['atasan_id'],
                    'new_bawahan',
                    'Bawahan Baru',
                    "{$user->name} kini menjadi bawahan Anda.",
                    ['employee_id' => $user->id, 'employee_name' => $user->name]
                );
            }

            // ✅ NOTIFIKASI: Kirim ke atasan lama jika berubah
            if ($oldAtasanId && $oldAtasanId !== ($validated['atasan_id'] ?? null)) {
                NotificationHelper::create(
                    $oldAtasanId,
                    'bawahan_removed',
                    'Bawahan Dipindahkan',
                    "{$user->name} tidak lagi menjadi bawahan Anda.",
                    ['employee_id' => $user->id, 'employee_name' => $user->name]
                );
            }
    ...
    private function buildHierarchy($user)
    {
        ...
        return [
            ...
            'posisi' => $user->posisi,
            'picture' => $user->picture_url, // Gunakan accessor
    ...
    public function toggleStatus($id)
    {
        ...
        // ✅ NOTIFIKASI: Kirim ke karyawan
        $statusText = $user->is_active ? 'diaktifkan' : 'dinonaktifkan';
        NotificationHelper::create(
            $user->id,
            'status_changed',
            'Status Akun Berubah',
            "Status akun Anda telah {$statusText}.",
            ['employee_id' => $user->id, 'is_active' => $user->is_active]
        );
